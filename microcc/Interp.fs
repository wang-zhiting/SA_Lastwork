...
let rec exec stmt (locEnv: locEnv) (gloEnv: gloEnv) (store: store) : store =
    ...
    | For (e1, e2, e3, body) ->
        ...
        let rec loop store1 = 
            ...
            if v <> 0 then
                ...
                //执行一次判断条件变更
                let (res2, store4) = eval e3 locEnv gloEnv store3
                loop store4
            else
                store2 //退出循环返回 环境store2
        loop store0
    ...
    |DoWhile (body, e) ->
        ...
        let store0 = exec body locEnv gloEnv store
        loop store0
    ...
    | Switch (e,body) ->
        ...
        let rec chooseCase caseList store1=
            match caseList with
            | [] -> store1
    ...
    | ForRange (acc,rangeList,body) ->
        let (loc, store0) = access acc locEnv gloEnv store
        let len = rangeList.Length
        let (min,max,step) =
            match len with
                | 1 -> 
                    (0,rangeList.Item(0),1)
                | 2 ->
                    (rangeList.Item(0),rangeList.Item(1),1)
                | 3 ->
                    (rangeList.Item(0),rangeList.Item(1),rangeList.Item(2))
                | _ -> (0,0,0)
        let rec loop store1 m =
            let store2 = setSto store1 loc m
            if m<max then
                //执行一次内部语句
                let store3 = exec body locEnv gloEnv store2
                //执行一次判断条件变更
    ...
    | Return (Some e) -> 
        ...
        let loc = store1.Count
        let store2 = setSto store1 loc res
        store2
...
and stmtordec stmtordec locEnv gloEnv store =
    ...
    | DecAndAssign (typ, x, e) -> 
        ...
        let (loc, store3) = access (AccVar x) locEnv1 gloEnv store1
        let (i, store4) = eval e locEnv gloEnv store3
        (locEnv1,setSto store4 loc i)
...
and eval e locEnv gloEnv store : int * store =
    ...
    | AssignEQ (ope, acc, e) ->
        //执行n+=1,n是一个Access
        let (loc, store1) = access acc locEnv gloEnv store//取得acc的地址放入loc
        let (i1) =  getSto store1 loc//取得loc地址代表的值 即n的值
        let (i2, store2) = eval e locEnv gloEnv store//取得e的值
        let res =
            match ope with
            | "+=A" -> i1 + i2
            | "-=A" -> i1 - i2
...
and callfun f es locEnv gloEnv store : int * store =
    ...
    let loc = store3.Count-1
    let res = getSto store3 loc
    let store4 = store3.Remove(loc)
    (res, store4)
